...
Foundation code is **built, unit-tested, and validated on the real 1000G BAMs**. Both tensor datasets are **extracted and validated**: **11,016 labeled windows** (train chr1–11 = 7,672; test chr12–22 = 3,344) and **80,000 unlabeled SSL windows** (749 MB). **SSL pretraining is COMPLETE** — job 1514837 finished cleanly (exit 0:0, 25 epochs, 8h14m wall, final loss 40.8→15.2), encoder saved to `ckpt/encoder_ssl.pt`. **Fine-tune + label-efficiency sweep is RUNNING across all 3 free T4 GPUs** (jobs **1515265/66/67 = seeds 0/1/2**), each doing the full 6-fraction pretrained-vs-scratch sweep + calibration + length-strata → error bars on the money plot. **Headline result already reproduces cleanly across all 3 seeds** (see below). NA12878 (CEU) cross-population BAM downloaded (250.9 GB) and indexed.
...
## 🔑 HEADLINE RESULT (partial sweep, 3 seeds — reproduces cleanly)

Test set = chr12–22, 3,344 windows. Numbers are F1 on the DEL-vs-non-DEL task.

| Label fraction | Pretrained F1 (seed 0 / 1 / 2) | From-scratch F1 (seed 0 / 1 / 2) |
|---|---|---|
| **1%** (~77 ex.) | 0.459 / 0.433 / 0.309 | **0.000 / 0.000 / 0.000** |
| 5% | 0.331 / 0.527 / 0.364 | 0.537 / 0.177 / 0.108 |
| 10% | 0.492 / 0.587 / 0.610 | 0.631 / 0.782 / 0.751 |
| 25% | 0.622 / 0.688 / 0.720 | _(running)_ |
| 50% | _(running)_ | _(running)_ |
| 100% | _(running)_ | _(running)_ |

**Interpretation:** at **1% labels the from-scratch model collapses to F1 = 0 in all three seeds**, while the pretrained encoder still recovers real deletions (F1 ≈ 0.31–0.46). This is the project's central claim — SSL pretraining rescues the extreme low-label regime where supervised training fails. The advantage narrows (and at 5–10% can reverse) as labels increase, which is the expected label-efficiency signature and the honest story for the paper. Full curve + error bars pending the 50%/100% fractions.

**NOT YET DONE (do not overclaim):** no head-to-head vs an actual DeepSV baseline yet (Phase 0.3). "Beats DeepSV" is unproven; current claim is strictly "pretrained vs from-scratch, same architecture."
...
| 2.3 | **Label-efficiency "money plot"** (pretrained vs scratch, 1–100% labels) | 🟡 running | Jobs **1515265/66/67** (seeds 0/1/2) on 3× T4, full sweep 0.01–1.0 + calibration + length-strata; error bars from 3 seeds. **1% result confirmed: pretrained F1 0.31–0.46 vs scratch 0.000 across all seeds.** 50%/100% fractions still computing. |
...
2. 🟡 **Fine-tune + label-efficiency sweep** (jobs **1515265/66/67**, 3× T4, seeds 0/1/2): each runs the full pretrained-vs-scratch sweep (fracs 0.01/0.05/0.1/0.25/0.5/1.0) + temperature calibration + ECE + length-stratified recall (amendment 1). ← _running now; through 25% fraction, 50%/100% pending. 1% headline confirmed (pretrained 0.31–0.46 vs scratch 0.000)._
3. **Aggregate the 3 seeds** → the label-efficiency money plot **with error bars** (pretrained vs scratch), calibration table, length-strata table. Publication-grade figure.
4. **Cross-population eval**: extract NA12878 (CEU) test tensors once indexed (1514991), score the fine-tuned model on a held-out individual + ancestry.
5. Phase 0.3 DeepSV-like supervised baseline for the head-to-head F1 claim.
