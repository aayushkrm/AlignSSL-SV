...
**Verdict:** MAE-only wins 5/6 fractions, best calibration (ECE 0.010), tightest variance (frac=1.0 std 0.006 vs combined 0.031). Combined (original design pick) is worst/near-worst at low fractions, mid-pack at 100%, and its long-DEL recall is low AND unstable (0.578±0.291, one seed collapsed). VICReg-only recovers long DELs but lags on F1 everywhere.
