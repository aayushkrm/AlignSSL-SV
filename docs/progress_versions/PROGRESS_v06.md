...
Foundation code is **built, unit-tested, and validated on the real 1000G BAMs**. Both tensor datasets are **extracted and validated**: **11,016 labeled windows** (train chr1–11 = 7,672; test chr12–22 = 3,344) and **80,000 unlabeled SSL windows** (749 MB). **SSL pretraining is COMPLETE** — job 1514837 finished cleanly (exit 0:0, 25 epochs, 8h14m wall, final loss 40.8→15.2), encoder saved to `ckpt/encoder_ssl.pt`. **Fine-tune + label-efficiency sweep is now RUNNING in parallel across all 3 free T4 GPUs** (jobs 1514992/93/94 = seeds 0/1/2), each doing the full 6-fraction pretrained-vs-scratch sweep + calibration + length-strata → gives error bars on the money plot. NA12878 (CEU) cross-population BAM downloaded (250.9 GB) and re-indexing (job 1514991).

**SSL launch bug chain resolved today (all real, all fixed):** (1) DataLoader reading 40× 1.2 GB compressed shards starved the GPU / risked worker hangs → consolidated into one 47 GB float16 memmap (`build_memmap.py`), staged to `/dev/shm`, `num_workers=0` → GPU pins 100%; (2) batch 256 OOM'd the 15 GB T4 → batch 96 + `expandable_segments:True`; (3) `is_bf16_supported()`=True on Turing but bf16 is emulated & ~4× slower → select fp16 by compute capability (<sm_80). Now ~1.5 s/step, ~21 min/epoch.
...
| 2.3 | **Label-efficiency "money plot"** (pretrained vs scratch, 1–100% labels) | 🟡 running | Jobs 1514992/93/94 (seeds 0/1/2) on 3× T4, full sweep 0.01–1.0 + calibration + length-strata; error bars from 3 seeds |
...
2. 🟡 **Fine-tune + label-efficiency sweep** (jobs 1514992/93/94, 3× T4, seeds 0/1/2): each runs the full pretrained-vs-scratch sweep (fracs 0.01/0.05/0.1/0.25/0.5/1.0) + temperature calibration + ECE + length-stratified recall (amendment 1). ← _running now, ~1–2 h_
