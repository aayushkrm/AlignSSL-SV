...
Foundation code is **built, unit-tested, and validated on the real 1000G BAMs**. Both tensor datasets are **extracted and validated**: **11,016 labeled windows** (train chr1–11 = 7,672; test chr12–22 = 3,344) and **80,000 unlabeled SSL windows** (749 MB). **SSL pretraining is TRAINING on gpu_T4 (job 1514837)** — fp16, 25 epochs, ~8.5 h, loss annealing from 40.8; encoder saved every epoch to `ckpt/encoder_ssl.pt`. (A100 was locked for 7–9 days by other users, so redirected to the free T4.) NA12878 cross-population download resumes in the background (job 1514788).
...
| # | Task | Status |
|---|------|--------|
| 2.1 | MAE objective pretraining on 1000G | 🟡 training | Combined MAE+VICReg, job 1514837 on gpu_T4 (25 epochs, fp16, batch 96, 80K windows via shm memmap). Loss 40.8→annealing |
| 2.2 | Contrastive/VICReg objective + coverage & cross-sample views | 🟡 training | VICReg runs jointly with MAE in 1514837 (w_mae=1, w_vicreg=1); dedicated view ablations later |
...
1. **SSL pretraining** (job 1514837, `gpu_T4`): MAE+VICReg on 80K unlabeled windows, 25 epochs → `ckpt/encoder_ssl.pt`. ← _training now, ~8.5 h_
2. **Fine-tune + calibrate** (`finetune_eval.sbatch`, `gpu_T4`) → the label-efficiency money plot (pretrained vs scratch, 1–100 % labels) + temperature calibration + length-stratified recall (amendment 1). Launches once 1514837 writes the encoder.
