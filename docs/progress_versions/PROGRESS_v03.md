...
Foundation code is **built, unit-tested, and validated on the real 1000G BAMs**. BAM indexing is done; the **full labeled + unlabeled tensor extraction is running on the cluster** (job 1514620 running, 1514621 queued). NA12878 download is in the background (~35%).
...
| A11 | NA12878 (CEU) download for cross-population test | 🟡 | Job 1514554, ~35% (87.5 GB / ~250 GB) |
...
| 0.1b | Full labeled extraction (both samples, all chroms) | 🟡 | Job 1514620 running: NA19238=5,484 items (1,371 pos), NA19625=5,532 (1,383 pos) → ~11K labeled windows |
| 0.1c | Unlabeled pretrain-window extraction (train chroms) | 🟡 | Job 1514621 queued (2×80K windows); auto-starts when 0.1b frees CPUs |
