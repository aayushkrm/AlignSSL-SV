# Three defects in the deep structural-variant evaluation design, and what survives when they are corrected
...
**That result does not survive scrutiny, and neither do several others.** We identify three defects in the evaluation design and correct each:
...
**Conclusion.** We report this as a methodological result rather than a method paper. Each defect is individually mundane and each is, we believe, widespread: benchmarks built from randomly-sampled negatives, F1 reported at a fixed cut, and label budgets computed per-arm are all standard practice in this literature. Together they were sufficient to produce a confident, statistically significant, entirely artefactual headline. We publish the corrections, the controls, and the code that implements them so that the next paper in this lineage can be checked against them.
...
What follows is therefore an audit rather than a method paper. We report the framework because the corrections are only legible against a concrete instance of the design, and because the same three defects are, as far as we can determine, standard practice in this literature rather than idiosyncratic to our implementation. Our contributions are:
...
All three rules live in a single module (`alignssl/protocol.py`) imported by every evaluator, with unit tests, so the arms cannot drift apart again. All results in Sections 4.8 and 4.2's Table 14 use this protocol; Sections 4.1–4.7 and 6 predate it and are labelled accordingly.
...
![Figure 1. Deletion F1 vs. labelled-data fraction as conventionally scored, at a fixed 0.5 probability cut. The apparent low-label dominance of the pretrained arm is an artefact of that cut; see Figure 8.](/Users/akm/.claude-science/orgs/d8cb02ba-8b97-435d-8e48-98fb5d9a2948/artifacts/proj_acb816763367/6b3657d8-5b3f-4b2c-b2ad-1df2139e7a24/v56205422_figure1_label_efficiency.png)
...
From the same alignment tensors, identical shards, identical chromosome split, identical label fractions and identical seeds, we computed twelve scalar summary features per window — mean, standard deviation and minimum of the depth profile; centre-versus-flank depth ratio; maximum sustained depth drop; discordant-pair rate; soft-clip rate; mean and maximum insert-size |z|; mean mapping quality; occupied read-row count; and valid-base fraction — and fitted an ℓ2-regularised logistic regression and a gradient-boosted tree (`Classical-logreg`, `Classical-GBT` in Table 1). The tree is at its ceiling from the smallest label budget onward. At 1% labels (210 windows), where the paper's headline claim lives, it reaches **AUPRC = 0.937 ± 0.009** against 0.524 ± 0.052 for the pretrained network and 0.427 ± 0.138 from scratch. Increasing its labels one hundred-fold, to the full 21,016 windows, buys it **+0.038** (0.975 ± 0.001). A benchmark on which twelve scalar features are 96% of the way to their asymptote after 210 examples cannot discriminate between representation-learning methods, because there is almost nothing left for a representation to contribute. Its variance across seeds is one to two orders of magnitude smaller than any deep arm's.
...
![Figure 8. Pretrained-to-scratch ratio by label budget under two thresholding rules (left), and absolute scores at the smallest budget under all three (right). The ratio departs from parity at exactly one budget under exactly one rule.](/Users/akm/.claude-science/orgs/d8cb02ba-8b97-435d-8e48-98fb5d9a2948/artifacts/proj_acb816763367/72cb364e-16f1-446e-9c35-db499a6b8fb3/vb8dffbbf_figure8_threshold_sensitivity.png)
...
> **Provisional.** Every number in this subsection was produced under the two conventions that Sections 3.8 and 4.8 subsequently show to be defective: unequal label budgets across arms, and F1 at a fixed 0.5 probability cut. Finding (i) below is a fixed-cut contrast of exactly the kind Section 4.8 demonstrates to be unreliable, and finding (iii) is a form of the claim Table 14 already corrects on the uniform benchmark. The corrected re-run of this benchmark was still in progress at the time of writing. We retain the subsection because the *benchmark-construction* result — that quantile-matched candidate negatives attenuate the depth shortcut from ROC-AUC 0.955 to 0.717 without changing the positive set — does not depend on either convention and stands. The arm-versus-arm contrasts below do not, and no claim elsewhere in this paper rests on them.

**(i) The pretrained-versus-scratch gap survives and sharpens.** At 1% of labels (96 windows) the pretrained model reaches F1 = 0.352 ± 0.064 while the identically-architected from-scratch model is degenerate at 0.000 ± 0.000 — it never fires (*p* = 0.016). The same holds at 5% (0.325 vs 0.000). On the uniform benchmark the from-scratch model was weak at 1% but not degenerate; on the harder task it fails outright below 10% labels. Read at face value this would make the claim that self-supervised initialisation is what makes a low-label detector trainable *stronger* here than in Section 4.1. Section 4.8 is the reason not to read it at face value: an arm scoring 0.000 at a fixed cut is precisely the signature of a model whose probabilities sit below the cut, which is compatible with competent ranking and was in fact what the uniform benchmark's apparent collapse turned out to be.
...
**(iii) A hand-crafted control still leads at every label budget.** Gradient-boosted trees on twelve scalar features lead at 10%, 25%, 50% and 100%; logistic regression on the same features leads at 1% and 5%. The central objection to this paper therefore appeared to stand after the fix, and to stand more informatively: not an artefact of uniformly-sampled negatives alone. Table 14 qualifies this: on the uniform benchmark, scored threshold-free and at equal budgets, the control's lead is significant at only 2 of 6 budgets and reverses at full supervision. Whether the same qualification applies here awaits the corrected re-run. The one qualification is that the gap closes at full supervision — 0.791 for GBT versus 0.762 for the pretrained tensor model is not a significant difference at three seeds (*p* = 0.741) — and that the GBT is itself degenerate at 1% labels (0.000), where it is the pretrained network and the logistic regression that still produce a usable detector.
...
- **Statistical power.** Three to four pretraining seeds per arm are not sufficient to rank the self-supervised objectives against one another (*p* > 0.2, Section 4.5), to establish cross-ancestry transfer (significant at 1 of 6 label fractions, Section 4.6), or — as Section 4.8 shows — to distinguish the pretrained and from-scratch arms at any budget once scoring is threshold-free. Those analyses are reported as orderings of means, not findings. The one contrast that *did* reach significance at this seed count is the one we withdraw, which is itself a caution: a small-*n* design that produces a *p* = 0.009 under one scoring rule and *p* = 0.348 under another has not measured a stable effect.
...
- **Candidate filtering is partial, and Section 6 is not yet re-run.** The repaired benchmark of Section 6 attenuates the depth shortcut to ROC-AUC 0.717 but does not eliminate it, and it is single-sample (NA20845 train/test, NA12878 held out) with 1,516 test windows, because a mid-study loss of the shared reference directory left only two of the six panel alignments recoverable. Its conclusions are correspondingly wider-error-barred than Section 4's. More importantly, **the numbers reported in Section 6 predate the protocol and scoring corrections of Sections 3.8 and 4.8** and are flagged there as provisional; the corrected re-run is in progress at the time of writing. Readers should not treat any Section 6 contrast between the pretrained and from-scratch arms as established.
...
It fails three controls, none of which is exotic:
...
Each defect is individually mundane, each is widespread in this literature, and each is invisible without the control that exposes it. Jointly they were sufficient to manufacture a large, statistically significant, entirely artefactual headline result — one we believed, wrote up, and would have submitted.
...
We therefore do not offer a new SV caller, and we make no performance claim. We offer three controls, the code that implements them, and a worked demonstration of what their absence costs. Before the next architecture is proposed for short-read deletion calling, we would ask of it the three questions this paper failed: what does a hand-crafted-feature model score on the same windows, do all arms receive the same labels, and does the result survive a change of decision threshold.
