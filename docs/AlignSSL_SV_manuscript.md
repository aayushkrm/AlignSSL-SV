...
**Table 1. Label efficiency as conventionally scored (deletion F1 at a fixed 0.5 probability cut, test chr12–22, batch 96, 3 seeds per arm). Table 2 reports the same runs under threshold selection and threshold-free scoring; the two tables disagree, and Table 9 quantifies the disagreement.**
...
We train on the in-distribution panel and evaluate both in-distribution and on an entirely held-out population (CEU), sweeping the full label fraction (Table 8). These runs predate the label-accounting correction of Section 3.8, and for a label sweep that matters: the low-fraction cells received a batch-size-floored budget rather than the nominal share, so they overstate how little supervision each arm consumed. We report the sweep as it was run and claim nothing from it. We also did not re-run it under the corrected protocol, and that decision cuts both ways: the correction reversed the direction of the headline label-efficiency contrast (Section 3.8), so it could in principle turn this section's null into an effect as easily as leave it null. The null below is therefore provisional in both directions, and a corrected re-run — `scripts/cross_pop_lowlabel.py` is already migrated to the shared protocol — is the cheapest of this paper's open items. Reporting only the extremes, as an earlier version of this analysis did, conceals the structure of the result; we therefore give all six fractions.
...
**Table 9. The 1%-label contrast under three scoring rules (uniform benchmark, 210 labels, 4 seeds per arm; *p* from Welch's *t*-test over seeds).**
...
We therefore pre-declared families — one per sweep, per contrast, per scoring rule — and applied both Holm–Bonferroni, which controls the family-wise error rate, and Benjamini–Hochberg, which controls the false discovery rate (`analysis/apply_multiplicity.py`, output `results/stats_multiplicity.csv`). Across 67 tests in 11 families, 25 are nominally significant and 17 survive Holm.

**Table 10. Claims whose significance does not survive family-wise correction (Holm–Bonferroni at α = 0.05 within the stated family). Full output in `results/stats_multiplicity.csv`.**
...
**Table 12.** Untrained single-feature separability, uniform versus candidate-filtered negatives (orientation-corrected ROC-AUC; source `results/table9_hardneg_single_feature_auc.csv`).
...
**Table 14. Best control arm versus best deep arm on the caller-candidate
