...
**Motivation.** Deep learning is now the dominant paradigm for structural-variant (SV) detection from short-read sequencing, and the field inherits from DeepSV (Cai, Wu & Gao, 2019) not only an architecture — a convolutional network over hand-designed RGB pileup images — but an *evaluation design*: negatives sampled at random from the genome, F1 reported at a fixed 0.5 probability cut, label budgets computed per arm, and significance read off a sweep of budgets as though it were a single test. Those four conventions have propagated with the architecture and, as far as we can determine, have not been checked. This paper checks them. We do so by building the method the inherited framing recommends — a learned alignment representation with self-supervised pretraining, targeting the data hunger, miscalibration and ancestry brittleness that framing leaves open — measuring it by the field's conventions, obtaining a large and significant positive result, and then subjecting that result to the controls the conventions omit. The controls are the contribution; the method is the instrument that makes them legible.

**Results.** We present AlignSSL-SV, a framework that (i) replaces the fixed RGB pileup with a multi-channel alignment tensor and a learned encoder, (ii) pretrains that encoder by masked-alignment modelling (a self-supervised objective on read alignments, requiring no SV labels), and (iii) attaches a calibrated deletion head. Evaluated on 1000 Genomes high-coverage PCR-free data (a six-sample panel spanning five continental ancestries) under the conventions this literature inherits from DeepSV, the framework appears to deliver a large low-label gain: deletion F1 of 0.46 at 1% of labels (210 windows) against 0.11 for an identically-architected from-scratch model, a 4.38× improvement attributable to self-supervised initialisation alone (*p* = 0.0002, and — unlike most effects in this literature — one that survives correction for the six-budget sweep it was selected from).
...
**Availability.** Code, the tensor-extraction pipeline, result tables, figures, are at https://github.com/aayushkrm/AlignSSL-SV (MIT licence); all 22 pretrained encoder checkpoints are released at `/releases/tag/v0.1.0-weights` and indexed by `release/WEIGHTS_MANIFEST.tsv`, which records for each one whether it backs a reported number.
...
- A controlled evaluation on a six-sample, five-ancestry 1000 Genomes panel which, scored by the conventions this literature inherits, shows a large low-label gain from pretraining (4.38× F1 at 1% labels, *p* = 0.0002) — the result the rest of the paper re-scopes to a statement about calibration rather than about representation quality (Section 4).
...
Table 1 reports that F1 as a function of the fraction of the labelled training set made available to the fine-tuning head. The apparent result is in the **low-label regime**: at 1% of labels (210 windows), the pretrained model reaches F1 = 0.464 ± 0.087, whereas the identically-architected from-scratch model all but collapses to 0.106 ± 0.139 — a **4.38× improvement in F1** from self-supervised initialisation alone, at *p* = 0.0002. Unlike the pre-correction version of this table, that *p* survives family-wise correction across the six budgets it was selected from (Section 4.9); the effect is real, and the question Section 4.8 answers is what it is an effect *on*. As labels increase the from-scratch model catches up and overtakes; by 5% it is already ahead (0.689 vs 0.605).

Two features of the table are worth noting before it is dismantled, because neither is compatible with the story it is usually told to support. First, the DeepSV-representation baseline does not merely match but slightly *exceeds* the pretrained tensor model at 1% (0.473 ± 0.042 vs 0.464 ± 0.087) — whatever the pretrained arm is doing at the smallest budget, a hand-designed RGB encoding does it at least as well, which already argues against a representation-quality explanation. The 4.38× advantage is over the from-scratch arm alone and over no other arm in the table. Second, that arm's collapse is confined to a single cell: one budget later it leads every arm in the table. A representation deficit that vanishes between 210 and 1,051 labels is a strange kind of deficit.

Read on its own, this table says that pretraining supplies a usable detector from a truth set two orders of magnitude smaller than is conventionally required. It says nothing of the kind. Section 4.2 shows the benchmark is separable without any model, and Section 4.8 shows the 4.38× gap is a property of the threshold rather than of the representation — the from-scratch model at 210 labels ranks essentially as well as the pretrained one (AUPRC 0.495 vs 0.504) and merely places its probabilities below 0.5 while doing it.

**Table 1. Label efficiency as conventionally scored (deletion F1 at a fixed 0.5 probability cut, test chr12–22, batch 96; AlignSSL-pretrained 4 seeds, AlignSSL-scratch and DeepSV-repr. 10 each — the pretrained arm is limited by the number of pretraining runs, source `results/table12_label_efficiency_fixed.csv`). Table 2 reports the same runs under threshold selection and threshold-free scoring; the two tables disagree, and Table 10 quantifies the disagreement.**

| Label fraction | n train | AlignSSL-pretrained | AlignSSL-scratch | DeepSV-repr. |
|---|---|---|---|---|
| 1% | 210 | 0.464 ± 0.087 | 0.106 ± 0.139 | **0.473 ± 0.042** |
| 5% | 1,051 | 0.605 ± 0.072 | **0.689 ± 0.115** | 0.496 ± 0.086 |
| 10% | 2,102 | 0.749 ± 0.037 | **0.788 ± 0.066** | 0.570 ± 0.113 |
| 25% | 5,254 | 0.808 ± 0.088 | **0.854 ± 0.080** | 0.720 ± 0.099 |
| 50% | 10,508 | 0.860 ± 0.061 | **0.894 ± 0.066** | 0.785 ± 0.061 |
| 100% | 21,016 | **0.895 ± 0.046** | 0.880 ± 0.067 | 0.696 ± 0.159 |

![Figure 1. Deletion F1 vs. labelled-data fraction on the uniform benchmark, scored the way this literature scores it: a fixed 0.5 probability cut. Source is the `f1_at_half` columns of the corrected-protocol runs (`results/table12_label_efficiency_fixed.csv`, uniform rows), so the panel plots exactly the values tabulated in Table 1. The bracket marks the 4.38× pretrained-over-scratch ratio at the smallest budget; that ratio is a property of the fixed cut and not of the representation, and it does not survive threshold selection or threshold-free scoring (Figure 6). Note also that the pretrained arm does not lead the DeepSV baseline at this budget — the baseline is marginally ahead.]({{artifact:art_6b3657d8-5b3f-4b2c-b2ad-1df2139e7a24}})

**Table 2. The same runs as Table 1, rescored under the corrected protocol (uniform benchmark; AUPRC, threshold-free, so neither the batch-size label inflation of Section 3.8 nor the fixed 0.5 cut of Section 4.8 applies; AlignSSL-pretrained 4 seeds, all other arms 10; bold marks the leading arm per budget; source `results/table12_label_efficiency_fixed.csv`).**

| Label fraction | n labels | Classical-GBT | Classical-logreg | AlignSSL-pretr. | AlignSSL-scratch | DeepSV-repr. |
|---|---|---|---|---|---|---|
| 1% | 210 | **0.937 ± 0.009** | 0.920 ± 0.012 | 0.504 ± 0.060 | 0.495 ± 0.113 | 0.489 ± 0.032 |
| 5% | 1,051 | **0.958 ± 0.006** | 0.909 ± 0.042 | 0.718 ± 0.062 | 0.824 ± 0.118 | 0.608 ± 0.068 |
| 10% | 2,102 | **0.967 ± 0.004** | 0.918 ± 0.014 | 0.830 ± 0.051 | 0.905 ± 0.024 | 0.703 ± 0.075 |
| 25% | 5,254 | **0.971 ± 0.002** | 0.926 ± 0.005 | 0.911 ± 0.053 | 0.949 ± 0.024 | 0.839 ± 0.088 |
| 50% | 10,508 | **0.974 ± 0.002** | 0.927 ± 0.002 | 0.923 ± 0.049 | 0.969 ± 0.010 | 0.889 ± 0.032 |
| 100% | 21,016 | **0.975 ± 0.001** | 0.927 ± 0.001 | 0.962 ± 0.015 | 0.955 ± 0.068 | 0.894 ± 0.044 |

Read against Table 1, this is the whole argument of the paper in two tables. The ten-fold pretrained-over-scratch gap at 1% labels is 0.504 versus 0.495 here — a 1.02× ratio that does not clear significance (*p* = 0.853, Section 4.8) — and the twelve-feature control that Table 1 never included leads every deep arm at every budget. Both classical controls exceed every deep arm below 25% labels.
...
We initially reported this control as dominating every deep arm at every budget. Under corrected scoring that statement needs qualification, and Table 3 supplies it: on the uniform benchmark the control's lead is statistically significant at the four smallest budgets (1% through 25%) and shrinks to a tie at 50% and at full supervision, where the best deep arm reaches AUPRC 0.962 ± 0.015 against the tree's 0.975 ± 0.001 (*p* = 0.174). The control is never beaten at any budget on either benchmark. The defensible claim is therefore: **twelve hand-crafted features are never outperformed by any deep arm we trained, and they lead outright where labels are scarce — the regime self-supervised pretraining is proposed for.**
...
We train on the in-distribution panel and evaluate both in-distribution and on an entirely held-out population (CEU), sweeping the full label fraction. This section reports two runs of that design. Table 9 is the original, run before the label-accounting correction of Section 3.8 and scored at a fixed 0.5 cut; Table 8 is the same design re-run under the shared protocol and scored under all three rules of Section 4.8. An earlier draft of this manuscript declared the re-run the cheapest of its open items and did not perform it. It has since been performed, and it is the primary result of this section; Table 9 is retained as the pre-correction record because the two disagree in an instructive way.

**The corrected re-run.** Across 36 tests — six label fractions × three scoring rules × two evaluation sites — five are nominally significant and **none survives Holm correction within the family** (`results/table26_xpop_lowlabel.csv`, `results/stats_xpop_lowlabel.csv`). No cross-ancestry claim is supportable at any budget under any scoring rule, which is the same conclusion the pre-correction run reached but reached for a different reason and at a different cell.

**The pre-correction hit does not replicate, and its replacement is a threshold artefact.** Table 9's single nominal hit was at 10% labels (*p* = 0.028). In the corrected re-run that cell is not significant under any rule (held-out CEU: *p* = 0.500 at the fixed cut, 0.786 at tuned τ, 0.193 threshold-free), and the nominal hits relocate to 1% labels. That 1% hit is instructive rather than encouraging: on held-out CEU it is *p* = 0.011 at the fixed 0.5 cut, *p* = 0.139 at a tuned threshold, and *p* = 0.077 threshold-free, on the identical runs and seeds. It is the Section 4.8 mechanism reproducing inside this sweep — a from-scratch model at 1% labels scores F1 0.080 ± 0.073 at a fixed cut and 0.395 ± 0.047 once the cut is chosen, so the fixed-cut contrast is largely measuring where the two arms put their probabilities rather than how well they rank. We flag it as an independent replication of Section 4.8's finding on a different sweep, not as cross-ancestry evidence.

**Direction across the sweep.** On held-out CEU the ordering of means is not stable and depends on the scoring rule: pretrained leads at four of six budgets under both F1 rules, but at only two of six threshold-free, where from-scratch leads from 10% labels upward. Of the five nominal hits in the whole family, three favour **from-scratch** (10% AUPRC, 25% tuned-τ F1, 25% AUPRC, all in-distribution). We therefore claim no cross-ancestry effect, and note that the mean ordering that motivated the original claim is itself an artefact of the scoring rule.

**Power.** With three seeds, and given the variance decomposition of Section 4.9.1 — in which run-to-run noise at a fixed data split is the same size as split-to-split noise — this design detects a median 0.24 AUPRC. Every null in this section should be read as underpowered rather than as demonstrated equivalence. Reporting only the extremes, as an earlier version of this analysis did, conceals the structure of the result; we therefore give all six fractions.

**Table 8. Cross-ancestry transfer, corrected protocol (train in-distribution → test held-out CEU, 3 seeds), all six label fractions × three scoring rules. Supersedes Table 9. Means ± sd across seeds; *p* from Welch's *t*-test, pretrained vs. from-scratch; Holm adjustment is over the full family of 36 tests (both evaluation sites), of which these 18 held-out-CEU cells are half. **No cell survives correction**, and the sole cell with *p* < 0.05 (1% labels, fixed cut) loses significance under both threshold-free rules on the identical runs — the Section 4.8 threshold artefact reproducing inside this sweep. In-distribution rows and the generalisation-gap decomposition are in `results/table26_xpop_lowlabel.csv`; per-test statistics in `results/stats_xpop_lowlabel.csv`.**

| Labels | Rule | Pretrained | From-scratch | Leader | *p* | Holm *p* |
|---|---|---|---|---|---|---|
| 1% | F1 @ 0.5 | 0.494 ± 0.118 | 0.080 ± 0.073 | pretrained | 0.011 | 0.372 |
| 5% | F1 @ 0.5 | 0.623 ± 0.058 | 0.552 ± 0.095 | pretrained | 0.348 | 1.000 |
| 10% | F1 @ 0.5 | 0.713 ± 0.038 | 0.654 ± 0.124 | pretrained | 0.500 | 1.000 |
| 25% | F1 @ 0.5 | 0.719 ± 0.087 | 0.714 ± 0.139 | pretrained | 0.963 | 1.000 |
| 50% | F1 @ 0.5 | 0.693 ± 0.052 | 0.709 ± 0.109 | scratch | 0.832 | 1.000 |
| 100% | F1 @ 0.5 | 0.667 ± 0.093 | 0.716 ± 0.162 | scratch | 0.683 | 1.000 |
| 1% | F1 @ tuned τ | 0.493 ± 0.074 | 0.395 ± 0.047 | pretrained | 0.139 | 1.000 |
| 5% | F1 @ tuned τ | 0.660 ± 0.071 | 0.526 ± 0.083 | pretrained | 0.103 | 1.000 |
| 10% | F1 @ tuned τ | 0.688 ± 0.094 | 0.663 ± 0.108 | pretrained | 0.786 | 1.000 |
| 25% | F1 @ tuned τ | 0.720 ± 0.107 | 0.781 ± 0.055 | scratch | 0.444 | 1.000 |
| 50% | F1 @ tuned τ | 0.688 ± 0.125 | 0.646 ± 0.205 | pretrained | 0.781 | 1.000 |
| 100% | F1 @ tuned τ | 0.680 ± 0.182 | 0.807 ± 0.142 | scratch | 0.398 | 1.000 |
| 1% | AUPRC | 0.549 ± 0.100 | 0.378 ± 0.050 | pretrained | 0.077 | 1.000 |
| 5% | AUPRC | 0.725 ± 0.059 | 0.684 ± 0.133 | pretrained | 0.660 | 1.000 |
| 10% | AUPRC | 0.796 ± 0.035 | 0.836 ± 0.026 | scratch | 0.193 | 1.000 |
| 25% | AUPRC | 0.857 ± 0.035 | 0.901 ± 0.015 | scratch | 0.147 | 1.000 |
| 50% | AUPRC | 0.889 ± 0.030 | 0.903 ± 0.061 | scratch | 0.748 | 1.000 |
| 100% | AUPRC | 0.858 ± 0.079 | 0.928 ± 0.062 | scratch | 0.300 | 1.000 |
...
**Table 10. The 1%-label contrast under three scoring rules (uniform benchmark, 210 labels; 4 pretrained seeds against 10 from-scratch seeds; *p* from Welch's *t*-test over seeds, which does not assume equal group sizes; source `results/table13_threshold_sensitivity.csv`).**
...
We therefore pre-declared families — one per sweep, per contrast, per scoring rule — and applied both Holm–Bonferroni, which controls the family-wise error rate, and Benjamini–Hochberg, which controls the false discovery rate (`analysis/apply_multiplicity.py`, output `results/stats_multiplicity.csv`). Across 103 tests in 17 families, 30 are nominally significant and 17 survive Holm.

**Table 11. Claims whose significance does not survive family-wise correction (Holm–Bonferroni at α = 0.05 within the stated family). Full output in `results/stats_multiplicity.csv`.**

| Claim | Family size | Raw *p* | Holm *p* | BH *q* | Fate |
|---|---|---|---|---|---|
| Scratch ahead @5%, F1 at selected τ | 6 | 0.018 | 0.092 | 0.055 | lost to multiplicity |
| Scratch ahead @10%, F1 at selected τ | 6 | 0.013 | 0.075 | 0.055 | lost to multiplicity |
| Cross-ancestry transfer @10% labels, pre-correction run (Section 4.6) | 6 | 0.028 | 0.169 | 0.169 | lost to multiplicity |
| Cross-ancestry transfer, corrected re-run, best cell (Section 4.6) | 6 | 0.009 | 0.056 | 0.056 | lost to multiplicity |
| Pretrained vs scratch @1% (candidate-filtered), F1 at τ | 6 | 0.037 | 0.224 | 0.224 | lost to multiplicity |
...
First, **the headline claim survives multiplicity and fails anyway.** Corrected against the five other budgets it was selected from, the 1%-label advantage under the fixed cut holds comfortably (raw *p* = 0.0002, Holm *p* = 0.0012). This is the correct family: the claim was selected *by* being the strongest cell of a budget sweep, so the budget sweep is what it must be corrected against — a claim is corrected against the comparisons that could have produced it, not against a neighbouring set of different questions. The claim therefore passes the test this section applies. It fails the one Section 4.8 applies: the effect is real, reproducible and statistically robust, and it is an effect on where the model places its probabilities rather than on how well it ranks. Multiplicity correction is not what dismantles the headline; only the scoring rule is. We had expected the opposite, and report the finding as it came out.

Second, **the cross-ancestry effect is withdrawn outright, under both runs of the design.** Section 4.6 already declined to claim ancestry robustness on the grounds that the effect reached significance at only one of six label fractions. Correction settles it for the pre-correction run: that one fraction was what a family of six produces by chance (Holm *p* = 0.169, BH *q* = 0.169). The corrected re-run does not rescue it. That run tests 36 cells — six label budgets × three scoring rules × two evaluation sites — and its most extreme cell (in-distribution AUPRC at 25% labels) reaches raw *p* = 0.009. Its fate does not depend on how the family is drawn. Under this paper's declared convention — one family per sweep, per contrast, per scoring rule, so *m* = 6 — it is Holm *p* = 0.056; under the aggregator's stricter whole-sweep family (*m* = 36) it is Holm *p* = 0.339. Neither crosses 0.05, and no other cell comes closer, at any budget, rule or site. The held-out CEU cells are further back still: the best is raw *p* = 0.011, Holm *p* = 0.064 at *m* = 6 (`results/stats_xpop_lowlabel.csv`, `results/stats_multiplicity.csv`). Two independent runs of one design, corrected against the families they were each selected from, agree that the effect is not there. The ordering of means remains as reported; the significance claim does not.

Third, and pulling the other way, **the corrections' own findings are not immune, but the strongest of them hold.** The representation findings of Section 4.10 survive Holm within their own families: on the candidate-filtered benchmark the from-scratch arm's advantage over the DeepSV representation separates at the three largest budgets under both AUPRC and F1 at τ (Holm *p* < 0.001 throughout), and the pretrained arm's does likewise. What does *not* survive is any pretrained-versus-scratch contrast on that benchmark under any rule at any budget except one (AUPRC at 25%, Holm *p* = 0.016, and in the direction favouring *scratch*) — consistent with Section 6.3's conclusion that the two are tied there.
...
is present on both benchmarks and strengthens with label budget, and
self-supervision is not the mechanism — 22 of the 24 surviving cells are the
from-scratch arm, which carries no pretraining at all. A four-seed design remains
underpowered for 42 of the 48 non-separating cells, which bounds how strongly even
the positive part may be stated.
...
The obvious version of the rule fails. Retaining the windows with the most deletion-like (lowest) centre-versus-flank depth ratio does not remove the shortcut but **inverts** it: a genome-wide lower tail is dominated by mappability dropouts at ratio ≈ 0, whereas heterozygous deletions sit near 0.5, so a classifier learns "very low ratio ⇒ negative" and the feature remains diagnostic with its sign flipped. We checked this before building the benchmark, by simulating the selection under the ratio geometry the real data exhibits (positives a mixture of homozygous ratios near 0.05 and heterozygous near 0.5; the candidate pool dominated by copy-neutral windows near 1.0 with a mappability-dropout tail near 0): extreme-tail selection yields ROC-AUC 0.016 for the depth ratio, as separable as a uniform draw and merely reversed. That simulation is a committed test (`tests/test_match_strata.py`) that gates the extraction job, so the argument against tail selection cannot silently stop holding; it is a statement about the selection rule, not a measurement on sequencing data.

The rule we adopt instead is **quantile matching within each multi-scale bin**: for every positive, a negative is drawn whose depth ratio falls in the same quantile stratum of the candidate pool, with strata computed separately per window scale because the ratio's meaning depends on span (pooling scales would leak the label through span alone). Candidates come from a one-pass binned coverage profile per chromosome, scored by prefix sums so that exhaustive stride scanning is affordable and the pool genuinely contains mid-range candidates — a uniformly drawn pool does not, which is why the matching must draw from a scan rather than a sample. Two distinct estimators of the depth ratio matter here and must not be conflated. The extractor matches on a ratio read from the binned coverage profile by prefix sums; the classifiers, and every separability number we report, see a ratio recomputed from the depth channel of the assembled tensor. Matching is enforced on the former: the extractor reports the median |positive − negative| offset in its own matching statistic over all chromosome × scale groups and **aborts** if that median exceeds `--max-offset` (default 0.25), on the reasoning that a silently unmatched benchmark yields a plausible-looking but meaningless accuracy number. For the three extractions used here the median offsets were 0.0092 (NA20845 train, 77 groups), 0.0114 (NA20845 test, 75) and 0.0150 (NA12878 test, 75) — matched to roughly 1% of the ratio's range, well inside the guard (`results/table29_match_quality_offsets.csv`). What matching does *not* do is drive the tensor-side ratio to chance, and §6.2 reports what it actually achieves. Positives, window geometry, channel layout, multi-scale binning, chromosome split and shard format are unchanged. The question the benchmark asks becomes the one a caller actually faces: *given that a coverage anomaly was proposed here, is it a real deletion?*
...
**Table 14.** Depth-ratio separability on the training and test splits of both benchmarks (centre-versus-flank depth ratio recomputed from the tensor depth channel, no training; ROC-AUC orientation-corrected so that higher means more separable; source `results/table28_split_separability.csv`).

| Benchmark | Split | Windows | Deletions | ROC-AUC | Median (pos) | Median (neg) |
|---|---|---|---|---|---|---|
| Uniform | train | 21,016 | 5,254 | 0.948 | 0.536 | 1.000 |
| Uniform | test | 9,196 | 2,299 | 0.956 | 0.523 | 1.000 |
| Candidate-filtered | train | 3,452 | 863 | 0.715 | 0.525 | 0.723 |
| Candidate-filtered | test | 1,516 | 379 | 0.719 | 0.524 | 0.754 |
...
Table 15 reports the full arm set re-run under the candidate-filtering protocol at identical label budgets and seeds, scored threshold-free (AUPRC) and at a validation-selected threshold. The numbers below are the **corrected** ones: they were regenerated after the protocol and scoring fixes of Sections 3.8 and 4.8, so unlike the earlier version of this subsection they are directly comparable with Tables 12–14. Absolute performance falls for every arm relative to the uniform benchmark — the pretrained model from AUPRC 0.962 to 0.856 at full supervision, the gradient-boosted tree from 0.975 to 0.869 — confirming that the task is genuinely harder and not merely re-labelled.

**Table 15.** Candidate-filtered benchmark, corrected protocol: AUPRC (mean ± sd over 4 seeds for AlignSSL-pretrained and 10 for every other arm; source `results/table12_label_efficiency_fixed.csv`). Bold marks the best arm at each budget.

| Labels | *n* | AlignSSL (pretrained) | AlignSSL (scratch) | DeepSV repr. | Classical logreg | Classical GBT |
|---|---|---|---|---|---|---|
| 1% | 35 | 0.300 ± 0.044 | 0.278 ± 0.042 | 0.316 ± 0.032 | **0.476 ± 0.076** | 0.250 ± 0.000 |
| 5% | 173 | 0.353 ± 0.066 | 0.364 ± 0.061 | 0.415 ± 0.026 | 0.596 ± 0.025 | **0.626 ± 0.054** |
| 10% | 345 | 0.451 ± 0.022 | 0.473 ± 0.093 | 0.434 ± 0.053 | 0.615 ± 0.026 | **0.719 ± 0.029** |
| 25% | 863 | 0.641 ± 0.026 | 0.724 ± 0.055 | 0.530 ± 0.043 | 0.624 ± 0.016 | **0.803 ± 0.013** |
| 50% | 1726 | 0.744 ± 0.045 | 0.803 ± 0.046 | 0.546 ± 0.071 | 0.629 ± 0.007 | **0.845 ± 0.012** |
| 100% | 3452 | 0.856 ± 0.035 | 0.836 ± 0.054 | 0.614 ± 0.047 | 0.631 ± 0.005 | **0.869 ± 0.006** |
...
**(i) The pretrained-versus-scratch gap does not survive threshold-free scoring here either. This is the second independent failure of the paper's original headline.** Under the corrected protocol the two arms are indistinguishable in AUPRC at five of six label budgets on the candidate-filtered benchmark — 0.300 ± 0.044 versus 0.278 ± 0.042 at 35 labels (*p* = 0.417), 0.856 ± 0.035 versus 0.836 ± 0.055 at full supervision (*p* = 0.431) — with the from-scratch arm nominally ahead in four of the six. The single exception runs *against* pretraining: at 25% labels the from-scratch arm is significantly ahead (0.724 ± 0.055 versus 0.641 ± 0.026, *p* = 0.003), the one cell in either benchmark where the pretrained-versus-scratch contrast separates under threshold-free scoring. The earlier version of this subsection reported the opposite — a gap that "survives and sharpens", with the from-scratch model degenerate below 10% labels — and Section 4.8 predicted exactly this outcome: an arm scoring near zero at a fixed 0.5 cut is the signature of a model whose probabilities sit below the cut, not of a model that cannot rank. The prediction is confirmed. At 35 labels the from-scratch arm's F1 is 0.090 at the fixed cut and identical, 0.090, at the "selected" threshold — the two coincide because 35 labels cannot form a validation split, so no selection actually occurs (see the protocol caveat below) — and this is the one budget where the fixed-cut gap (2.99×, *p* = 0.037) is not erased by threshold-free scoring, because no threshold selection was possible. Its AUPRC of 0.278 is nonetheless within noise of the pretrained arm's 0.300, so the ranking was never the deficient part. The uniform benchmark and the candidate-filtered benchmark agree: **on this task, self-supervised pretraining does not buy label efficiency in any ranking sense.**

**(ii) The learned tensor beats the DeepSV representation, and this finding strengthens under correction.** From 25% labels upward both tensor arms significantly exceed the RGB encoding under both scoring rules: at full supervision AUPRC 0.856 ± 0.035 (pretrained) and 0.836 ± 0.055 (scratch) against 0.614 ± 0.047 (*p* < 0.001 for both). This is the one Section 6.3 claim that the correction leaves intact and sharpens — on the uniform benchmark the corresponding contrast at full supervision is smaller. The interpretation is unchanged: the RGB encoding degrades more than the tensor does under candidate filtering, which is what one expects of a representation whose discriminative content was largely the coverage drop the protocol has now equalised. Note that the credit belongs to the *representation*, not to the pretraining — the from-scratch tensor arm carries this result as strongly as the pretrained one.

**(iii) The hand-crafted control leads at every budget but the last.** Table 3 reduces both families by best-of-family at each budget and tests the difference over seeds. On the candidate-filtered benchmark the control leads significantly at five of six budgets — AUPRC 0.476 ± 0.076 versus 0.316 ± 0.032 for the best deep arm at 1% (*p* < 0.001), 0.626 ± 0.054 versus 0.415 ± 0.026 at 5% (*p* < 0.001), 0.719 ± 0.029 versus 0.473 ± 0.093 at 10% (*p* < 0.001), 0.803 ± 0.013 versus 0.724 ± 0.055 at 25% (*p* = 0.001) and 0.845 ± 0.012 versus 0.803 ± 0.046 at 50% (*p* = 0.018) — with only full supervision a tie (0.869 ± 0.006 versus 0.856 ± 0.035, *p* = 0.514). Under the corrected protocol the control is never beaten at any budget on either benchmark. What this establishes — and it is the paper's most consequential negative result — is that **twelve scalar features are not outperformed by a pretrained convolutional–attention encoder anywhere on the curve, and beat it decisively wherever labels are scarce**, on a benchmark specifically constructed so that no single one of those features exceeds ROC-AUC 0.72.
...
**Table 16. Best control arm versus best deep arm on the caller-candidate
...
| 1,673 (50%) | Classical-GBT | 0.982 ± 0.002 | AlignSSL-scratch | 0.953 ± 0.010 | -0.029 | 0.0337 | **control** |
| 3,346 (100%) | Classical-GBT | 0.982 ± 0.001 | AlignSSL-scratch | 0.956 ± 0.010 | -0.026 | 0.0455 | **control** |
...
and leads significantly at five of the six budgets measured, including full
supervision.** It is worth
...
from reversing the withdrawal. It is one cell of a six-budget family, so we
...
*p* = 0.017, the only budget in the family to do so. It does not persist, and
past 10% labels it inverts: the from-scratch arm is marginally ahead at 25%
(0.933 versus 0.918, *p* = 0.268), ahead at 50% by more than the
uncorrected test tolerates (0.953 ± 0.010 versus 0.922 ± 0.002, *p* = 0.029;
Holm *p* = 0.143, so not significant within the family), and still ahead at
full supervision (0.956 ± 0.010 versus 0.939 ± 0.003, *p* = 0.098). Whatever pretraining
supplies at 33 labels is not merely absent by mid-range budgets — it is
reversed. And it is a contrast
...
**Table 17. Coded evaluation design of the 18 retrievable papers in the audit
...
- **No comparison against production callers.** We compare learned representations under matched conditions; we do not compare against Manta, DELLY, LUMPY, GRIDSS or similar. On the uniform and quantile-matched benchmarks such a comparison would favour whichever method best exploits the depth shortcut and would be uninformative. On the Section 6.5 benchmark it becomes meaningful - Manta's own score and PASS filter partition the same candidate pool we classify - and we flag it as the most direct comparison this design admits, left undone here because it requires re-deriving Manta's per-candidate scores under its own filtering logic rather than reading them from the pre-filter pool.
- **Statistical power and multiplicity.** Three to four pretraining seeds per arm are not sufficient to rank the self-supervised objectives against one another (*p* > 0.2, Section 4.5), to establish cross-ancestry transfer (nominally significant at 1 of 6 label fractions and not after correction, Sections 4.6 and 4.9), or — as Section 4.8 shows — to distinguish the pretrained and from-scratch arms at any budget once scoring is threshold-free. Those analyses are reported as orderings of means, not findings. The one contrast that *did* reach significance at this seed count is the one we withdraw, which is itself a caution: a small-*n* design that produces a *p* = 0.0002 under one scoring rule and *p* = 0.853 under another has not measured a stable effect, however small the first number is. A related limitation of the correction itself: Holm and BH assume the tests within a family are exchangeable, whereas budgets within a sweep share seeds, encoders and test windows and are therefore positively dependent. Both procedures remain valid under positive dependence (BH provably so, Holm conservatively), but the adjusted values should be read as conservative bounds rather than exact.
...
- **Truth set.** Sections 4 and 6.1-6.4 score against the 1000 Genomes phase-3 integrated SV call set, which is itself a consensus of short-read callers and therefore shares failure modes with the methods evaluated on it. Section 6.5 addresses this with an orthogonal, assembly- and long-read-derived truth set (GIAB HG002 Tier1 v0.6, confident regions only), and the depth shortcut reproduces there, so the paper's governing limitation is not an artefact of either truth set. What remains deferred is a *call-set-level* external validation - matching a produced VCF against GIAB with Truvari, including breakpoint-precision and genotype-concordance metrics - which requires a breakpoint-regression and genotype head this work does not have (Section 6.4). Section 6.5 scores candidate classification, not call-set recall.
...
One finding runs the other way, and we report it as such. On that third benchmark the pretrained arm beats the from-scratch arm at 33 labels by ROC-AUC 0.778 versus 0.545 (*p* = 0.0028; Holm *p* = 0.0168 within its six-budget family) — the only cell in the paper where the pretraining advantage is significant under a metric that is simultaneously threshold-free and base-rate-free, on negatives we did not choose. It does not persist past 10% labels — it reverses, with the from-scratch arm ahead at 50% and at full supervision — and both arms lose to the hand-crafted control at every budget, so it does not reinstate the headline. It is, however, the strongest surviving evidence that self-supervision buys label efficiency *relative to random initialisation*, and the natural target for a follow-up powered to test it. The corrections also soften the negative result at the top of the label range: on the uniform benchmark the control's lead over the best deep arm at full supervision is no longer significant (AUPRC 0.975 versus 0.962, *p* = 0.174), so the defensible claim is that the control is never *beaten*, not that it dominates significantly everywhere (Table 3).
...
The tensor-extraction pipeline, encoder and head implementations, pretraining and fine-tuning scripts, cluster job scripts, aggregation and manuscript-reconciliation tooling, result tables, and figures are available at https://github.com/aayushkrm/AlignSSL-SV under the MIT licence. All 22 trained encoder checkpoints are released as a single archive (`alignssl_sv_weights_v0.1.0.tar.gz`, 52,913,192 bytes, SHA-256 `9a62ea5199c51336779d57489587f56fe0f60f22103698eeb95106284e0eb84d`) at https://github.com/aayushkrm/AlignSSL-SV/releases/tag/v0.1.0-weights. `release/WEIGHTS_MANIFEST.tsv` in the repository lists, for every checkpoint, its SHA-256, encoder parameter count, embedding width, epoch count, last logged pretraining step, which pretraining corpus generation it belongs to, and which reported result — if any — it serves. Four checkpoints are the main MAM+VICReg encoders behind every reported pretrained arm; nine are objective ablations; eight are the statistic-anchored pretraining variant; one is a superseded first run. Checkpoints not used by any reported number are labelled as such in the manifest rather than omitted, and every one was verified to load into the released encoder class under `strict=True`. Sequencing data are from the 1000 Genomes Project (high-coverage PCR-free alignments, GRCh37/hs37d5) and are publicly available from the EBI 1000 Genomes FTP. The deletion truth set is the 1000 Genomes phase-3 integrated SV call set.
