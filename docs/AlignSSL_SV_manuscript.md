...
**Table 1. Label efficiency as conventionally scored (deletion F1 at a fixed 0.5 probability cut, test chr12–22, batch 96, 3 seeds per arm). Table 2 reports the same runs under threshold selection and threshold-free scoring; the two tables disagree, and Table 10 quantifies the disagreement.**
...
**Table 10. The 1%-label contrast under three scoring rules (uniform benchmark, 210 labels, 4 seeds per arm; *p* from Welch's *t*-test over seeds).**
...
**Table 15.** Candidate-filtered benchmark, corrected protocol: AUPRC (mean ± sd over 3 seeds for the deep arms, 10 for the classical controls; source `results/table12_label_efficiency_fixed.csv`). Bold marks the best arm at each budget.
