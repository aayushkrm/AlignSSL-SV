...
> **AS IMPLEMENTED (`alignssl/tensorize.py` default `max_rows=128`, but extraction run with `--max-rows 64`; verified shape of all result tensors `(18, 64, 256)`).** Built with **18 channels**, not 8–10 — the plan's minimum recommendation was expanded to include richer per-read encodings (separate one-hot planes for base identity rather than a single embedding channel, explicit CIGAR-derived indel-context channels, and a padding/validity mask channel) once real BAMs were in hand and channel budget was cheap on a `(18,64,256)` float16 tensor (~0.59 MB/window). `R=64` read rows (padded/masked) × `W=256` columns. Reference-base track, MAPQ, strand, insert-size z-score, and per-column depth are all present as planned; breakpoint offset labels are stored in the shard schema (`bp` key) but **not** consumed by any implemented loss (§2.1 scope reduction). Tensors are built with `pysam`, cached as compressed `.npz` shards (1024 items/shard) — matches the plan's "cache, don't build on the fly" guidance. Continuous channels are normalized with fixed constants (`isize_mean=450, isize_sd=100, depth_norm=60`) rather than training-set-computed statistics — a simplification worth revisiting if the panel expansion changes coverage/insert-size distributions materially.
...
> ⚠️ **SUPERSEDED (2026-07-24) by the hardened ablation.** The decision above rested on a confounded comparison: each ablation arm fine-tuned a **single shared seed-0 encoder** across 3 fine-tune seeds, so its error bars reflected fine-tune variance only, while the combined arm's bars spanned 4 **distinct pretraining seeds** — and the arms used different fine-tune batch sizes (128 vs 96). The pre-submission hardening DAG re-pretrained MAM-only and VICReg-only at seeds 1–2 (seed 0 reused) and re-fine-tuned every arm at batch 96, giving a clean single-variable comparison. The corrected result is a **crossover, not a "less-is-more" win**: MAM-only leads in the low-label regime (1%: 0.588 vs combined 0.514; 5%: 0.763 vs 0.655; 10%: 0.830 vs 0.813), but the **combined MAM+VICReg objective overtakes from 25% labels upward and is best at full supervision (100%: combined 0.934 vs MAM-only 0.915 vs VICReg-only 0.846)**. The **combined objective is therefore the adopted default** for panel-scale re-training and all moderate-to-full-supervision work; MAM-only is the recommended choice only when labels are extremely scarce. The "less-is-more" framing is withdrawn. See PROGRESS.md (2026-07-24) and `results/results_ablation_4arm_hardened.csv`.
...
>
> ⚠️ **SUPERSEDED (2026-07-24) — the numbers above are from the 2-sample pretrain corpus and the pre-hardening fine-tune configuration.** The final, authoritative sweep uses the 6-sample labeled panel (`tensors_all6`, train 21,016 / test 9,196 windows), the 3-sample / 120K-window SSL corpus, 4 pretraining seeds, and a harmonized batch size of 96 across every arm. Harmonized deletion F1: pretrained **0.514 ± 0.055** at 1% labels → **0.934 ± 0.004** at 100%; from-scratch **0.050 ± 0.040** → **0.944 ± 0.003**; DeepSV-representation baseline **0.434 ± 0.022** → **0.707 ± 0.140** (unstable at full supervision). The pretrained arm now beats the DeepSV baseline at **all six** label fractions, and both learned-tensor arms are ~10× better calibrated than it (ECE 0.008 / 0.007 vs 0.072). See PROGRESS.md (2026-07-24) and `results/results_label_efficiency_4seed.csv`.
