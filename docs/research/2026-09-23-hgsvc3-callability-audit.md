...
The official [working `20240307_PAV_VCF` directory](https://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/HGSVC3/working/20240307_PAV_VCF/) lists two ~12 GB tar archives, a README, and a manifest. Its [README](https://ftp.1000genomes.ebi.ac.uk/vol1/ftp/data_collections/HGSVC3/working/20240307_PAV_VCF/README_20240307_PAV_VCF) says these are intermediate calls from 65 phased assemblies and omit PAV-QC failures. The subsequent [reported member-header survey](2026-09-23-pav-callable-availability.md) reached the TAR end markers in both named archives and found no callable BED members. The survey did not inspect other working archives; its raw member log and runnable scanner still need archival for independent replication. Also, PAV's `_500` callable BED can bridge unaligned gaps, so BED membership alone is not a confident-negative criterion.

## Falsifiable negative-label gate

Do not turn a missing VCF record into a negative DEL label. Freeze candidate normalization and donor/haplotype masks first. A negative requires the full interval plus prespecified breakpoint flanks to be callable on **both** haplotypes, confirmed against unsmoothed reference-alignment intervals, with no assembly-QC filter, bridged unaligned gap, matching DEL, or overlapping complex event after representation-aware comparison. Missing haplotypes, gaps, QC-filtered intervals, and ambiguous representations remain **unknown**.
...
**Unresolved:** The v1.0 release inventories and two named `20240307_PAV_VCF` TARs do not provide a verified ready-to-use callable BED. Other HGSVC3 working archives, external assembly repositories, and a PAV rerun remain possible. PAV documents callable output, but availability of a validated donor-wide mask remains unverified.
